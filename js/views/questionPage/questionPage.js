...
$(".copyurlY").on({
  click: function () {
    ...
    if (window.location.href.indexOf('?') == -1) {
      let url = window.location.href.substring(0, window.location.href.length - 10) + $(this).parents(".queY").find("a").attr("href");
    } else {
      let url = window.location.href;
    }
    // console.log(url);
    let input = $("<input  value='" + url + "'>");
    $(this).prepend(input);
...
function readFile() {
  ...
  reader.onload = function (e) {
    ...
    // console.log(imgMsg);
    let newImage = template("templateImage", imgMsg);
    let imgObj = $(newImage);
    //把新的图片添加到编辑区
...
function loadMyNewAnswer(answerContents, answerId) {
  let type = $.cookie("level") != null ? "学生" : "教师";
  let school_info = type == "学生" ? $.cookie("major") : $.cookie("college");
